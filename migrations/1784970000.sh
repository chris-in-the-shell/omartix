...
#
# Reload rather than restart: elogind keeps active sessions intact while
# rereading its configuration.
as_root() {
  if (( EUID == 0 )); then
    "$@"
  else
...
as_root loginctl daemon-reload >/dev/null 2>&1 || true
...
dropin="${OMARTIX_ELOGIND_INHIBIT_DROPIN:-/etc/elogind/logind.conf.d/20-inhibit-delay.conf}"
