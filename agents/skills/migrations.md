...
Omartix has no pre-package-layout release to convert. Do not add a broad legacy
distribution conversion to the normal migration runner. If a future Omartix
release needs a one-time transition, design and test that migration explicitly
for Artix, dinit, and elogind.
