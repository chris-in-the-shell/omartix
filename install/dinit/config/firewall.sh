...
# UFW's dinit service is enabled by its Artix package. During ISO finalization
# configure persistent policy only; do not mutate the live installer's firewall.
...
install_ufw_docker_rules() {
  ...
  ufw_docker_bin=$(command -v ufw-docker) || return 1
  shim_dir=$(mktemp -d)
