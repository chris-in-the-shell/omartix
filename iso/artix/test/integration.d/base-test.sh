...
stop_vm() {
  ...
  if ! ssh_guest "echo $GUEST_PASSWORD | sudo -S loginctl poweroff" >/dev/null 2>&1; then
...
echo "$GUEST_PASSWORD" | sudo -S install -d /etc/dinit.d/boot.d
echo "$GUEST_PASSWORD" | sudo -S ln -srf /etc/dinit.d/sshd /etc/dinit.d/boot.d/sshd
echo "$GUEST_PASSWORD" | sudo -S dinitctl start sshd
EOF
...
# VM boots from. Sizing mirrors the configurator: 1MiB initial gap, 2GiB ESP,
# another 1MiB gap before root, then Btrfs minus the GPT backup reserve.
build_cidata() {
  local dir="$BASE_DIR/cidata" hash disk_encryption_config
  local disk_bytes=$((40 * 1024 * 1024 * 1024))
  local mib=$((1024 * 1024)) gib=$((1024 * 1024 * 1024))
  local boot_start=$mib boot_size=$((2 * gib))
  local main_start=$((boot_size + boot_start + mib))
...
,
        "disk_encryption": {
            ...
            "encryption_password": "$GUEST_PASSWORD"
        }
EOF
)
  else
...
install_phase() {
  ...
  local waited=0 text progress_name luks_passphrase_submitted=false
  while true; do
    ...
    # A normal encrypted installation deliberately has no embedded unlock
    ...
    if [[ ${OMARCHY_INTEGRATION_ENCRYPT_INSTALLATION:-true} == true && $luks_passphrase_submitted == false ]] &&
      ...
      type_text "$GUEST_PASSWORD"
      press ret
      luks_passphrase_submitted=true
    fi
