...
| Intel Low Power Mode Daemon (LPMD) | Not installed | Artix has no maintained `intel-lpmd` dinit service. Installing an Arch-oriented daemon without a supervisor would leave it inactive; normal power-profile selection remains available through `power-profiles-daemon-dinit`. | Artix packages `intel-lpmd` with a maintained dinit service and it is verified on supported Intel laptop hardware. |
