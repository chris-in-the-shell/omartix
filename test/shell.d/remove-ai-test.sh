...
# OpenClaw's active gateway is an Omartix dinit user service.  Upstream
# upstream units from pre-Omartix installations are removed as inert files;
# no legacy user manager is contacted.
cat >"$tmp_dir/bin/omarchy-openclaw-gateway" <<'SCRIPT'
#!/bin/bash
printf 'openclaw-gateway:%s\n' "$*" >>"$TEST_LOG"
[[ ${1:-} == remove ]] || exit 1
rm -f "$HOME/.config/dinit.d/boot.d/openclaw-gateway" \
  "$HOME/.config/dinit.d/openclaw-gateway"
SCRIPT
...
fresh_openclaw_home() {
  ...
  mkdir -p "$HOME/.config/dinit.d/boot.d" \
    "$HOME/.config/systemd/user/default.target.wants" "$HOME/.openclaw" \
    "$HOME/.local/share/applications" "$HOME/.local/share/icons/hicolor/256x256/apps"
  touch "$HOME/.config/dinit.d/openclaw-gateway" \
    ...
    "$HOME/.config/systemd/user/openclaw-node.service.bak" \
    "$HOME/.openclaw/openclaw.json" \
    "$HOME/.local/share/applications/OpenClaw.desktop" \
    "$HOME/.local/share/icons/hicolor/256x256/apps/openclaw.png"
  ln -s ../openclaw-gateway "$HOME/.config/dinit.d/boot.d/openclaw-gateway"
...
for gone in .config/dinit.d/openclaw-gateway \
  .config/dinit.d/boot.d/openclaw-gateway \
  ...
  .config/systemd/user/openclaw-node.service.bak \
  ...
  [[ ! -e $HOME/$gone && ! -L $HOME/$gone ]] || fail "OpenClaw removal deletes its dinit service, stale units, and launcher" "$gone"
done
grep -qx 'openclaw-gateway:remove' "$TEST_LOG" ||
  fail "OpenClaw removal delegates gateway teardown to Omartix dinit"
pass "OpenClaw removal deletes its dinit service, stale units, and launcher"

# Without a terminal there is nobody to ask, so the state stays. This avoids
# silently deleting the user's chats, memories, credentials, and plugins.
...
# A dinit teardown failure must abort before the package is dropped, otherwise
# an active gateway could outlive the package which supplied its executable.
cat >"$tmp_dir/bin/omarchy-openclaw-gateway" <<'SCRIPT'
#!/bin/bash
printf 'openclaw-gateway:%s\n' "$*" >>"$TEST_LOG"
exit 1
...
[[ $rc != 0 ]] || fail "OpenClaw removal aborts when the dinit gateway cannot be removed"
[[ -f $HOME/.config/dinit.d/openclaw-gateway ]] ||
  fail "OpenClaw removal aborts when the dinit gateway cannot be removed" "service file deleted"
! grep -q '^drop:openclaw$' "$TEST_LOG" ||
  fail "OpenClaw removal aborts when the dinit gateway cannot be removed" "package dropped anyway"
pass "OpenClaw removal aborts when the dinit gateway cannot be removed"
